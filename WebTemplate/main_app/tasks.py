...
def celery_email(pk: int, message_type: str, sender_id: int, status_label: str, item_related_user_ids: list[int]) -> None:
    ...
    notification_preferences = match_status_n_preferences(status_label)
    notification_ids = list(notification_preferences.values_list('user_id', flat=True))
    notifications = list(set(x for x in item_related_user_ids if x in notification_ids))
...
def some_night_task():
    # XXXXXX
    notifications = WebNotifications.objects.all()
